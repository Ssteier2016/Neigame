...
const botToken = '7473215586:AAHSjicOkbWh5FVx_suIiZF9tRdD59dbJG8'; // Token del bot de Telegram
...
// Nueva ruta para crear enlace de pago con Mercado Pago
app.post('/create-payment', async (req, res) => {
    ...
    try {
            ...
            items: [
                {
                    ...
                    quantity: 1,
                    currency_id: 'ARS' // Cambia a tu moneda (ej. MXN, BRL)
                }
            ],
            back_urls: {
                success: `https://neigame.onrender.com?payment=success&username=${username}&amount=${amount}`,
                failure: 'https://neigame.onrender.com?payment=failure',
                pending: 'https://neigame.onrender.com?payment=pending'
            },
        ...
        res.status(500).json({ success: false, detail: 'Error al procesar el pago' });
    }
});

// Nueva ruta para manejar notificaciones de Mercado Pago
    ...
    if (type === 'payment') {
        try {
            ...
            if (payment.status === 'approved') {
                ...
                if (users[username]) {
                    users[username].coins += amount;
                    io.emit('coins updated', { username, coins: users[username].coins });
                    console.log(`Pago aprobado: ${amount} ARS para ${username}`);
                }
            }
        } catch (error) {
